...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script is designed to perform a fast(!) searchlight on given atlases 
% labels. It loads predefined regions of interest,
...
function [results, center_indices, full_indices] = neurospark(roi1_pattern, roi2_pattern, analyser, mdata, opts)
...
opts.targetPropInVx = 1;
opts.searchlight_radius = 2;
opts.searchlight_padding = 2;
end
...
template = create_template(size(masks.roi1_mask), opts.searchlight_radius, opts.searchlight_padding);

% Load NIfTI image
nifti_image = niftiread(paths_nifti);

% Apply the target region mask to the NIfTI image
targetRoi = nifti_image .* masks.roi2_mask;
...
sourceRegion2D = extract2DRois(masks.roi1_mask, nifti_image);
...
[results, center_indices, full_indices] = searchlight(sourceRegion2D, ...
    opts.targetPropInVx, masks.roi2_mask, targetRoi2D, ...
